...
### Focus
- Core: Querying and transforming data with LINQ in C# applications.
- Senior Depth: Deferred execution, provider translation, and query performance trade-offs.
...
4. Arrays
5. Lists
6. Other IEnumerable Collections
7. Loading XML Data
8. Querying XML Data

### Study Notes (Simplified)
- LINQ offers composable querying over in-memory and remote data sources.
- Deferred execution improves composition but can surprise when source mutates.
- Query shape directly impacts generated SQL and performance with ORMs.

### Rapid Interview Q&A
- What is deferred execution in LINQ, and when can it cause bugs?
- Query syntax vs method syntax in LINQ: when should you use each?
- How does `IEnumerable` vs `IQueryable` change LINQ execution and performance?
...
public class Program
{
    ...
    public static void Main()
    {
        ...
        {
            new Student { Id = 1, Name = "Alice", Courses = new List<Course> { new Course { Title = "C#" }, new Course { Title = "SQL" } } },
            new Student { Id = 2, Name = "Bob", Courses = new List<Course> { new Course { Title = "C#" }, new Course { Title = "NoSQL" } } },
            new Student { Id = 3, Name = "Charlie", Courses = new List<Course>() } // Empty list
        };

        Console.WriteLine("=== 1. TRANSFORMATION (Select vs SelectMany) ===");
        
        // Select: Returns a List of Lists (Preserves structure)
        // Result: [ [C#, SQL], [C#, NoSQL], [] ]
        var listOflists = students.Select(s => s.Courses);

        // SelectMany: Flattens into a single list (One-to-Many)
        // Result: [ C#, SQL, C#, NoSQL ]
        var flatList = students.SelectMany(s => s.Courses).Select(c => c.Title);
        Console.WriteLine("Flat Courses: " + string.Join(", ", flatList));


        Console.WriteLine("\n=== 2. THE VALIDATION FAMILY (First vs Single) ===");

        // FirstOrDefault: Safe. Returns Alice or null if empty.
        var first = students.FirstOrDefault(s => s.Name == "Alice");

        // SingleOrDefault: SAFE if empty, but CRASHES if more than one match.
        // If we searched for Name == "Alice", it works. 
        // If we searched for Name.Contains("e"), it would CRASH because Alice and Charlie match.
        var single = students.SingleOrDefault(s => s.Id == 1); 

        // First: CRASHES if the list is empty. Use only when you are 100% sure data exists.
        var mustExist = students.First();


        Console.WriteLine("\n=== 3. LOGIC & PERFORMANCE (Any vs All) ===");

        // Any: Short-circuits. Stops as soon as it finds ONE match.
        // Performance: Much faster than .Count() > 0.
        bool hasCsharp = students.Any(s => s.Courses.Any(c => c.Title == "C#"));

        // All: Short-circuits. Stops as soon as it finds ONE failure.
        bool allHaveCourses = students.All(s => s.Courses.Count > 0); // False (Charlie is empty)
        Console.WriteLine($"Has C# Students: {hasCsharp}, All have courses: {allHaveCourses}");


        Console.WriteLine("\n=== 4. DEFERRED EXECUTION (The Ghost Query) ===");
        
        var query = students.Where(s => s.Name.StartsWith("A")); // Query defined here.
        
        students.Add(new Student { Id = 4, Name = "Alex", Courses = new List<Course>() }); 
        
        // The query executes NOW. It will include "Alex" even though he was added later!
        foreach(var s in query) Console.WriteLine("Found: " + s.Name);


        Console.WriteLine("\n=== 5. SET OPERATIONS (Distinct, Zip) ===");

        // Distinct: Removes duplicates from SelectMany
        var uniqueCourses = flatList.Distinct(); // [ C#, SQL, NoSQL ]
        
        // Zip: Merges two sequences like a zipper
        string[] grades = { "A", "B", "C" };
        var reportCard = students.Take(3).Zip(grades, (s, g) => $"{s.Name}: {g}");
        foreach(var line in reportCard) Console.WriteLine(line);
    }
}
```
