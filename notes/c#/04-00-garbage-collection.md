...
### Topic Map
1. Overview of garbage collection in the .NET framework
2. Introduction to Garbage Collection (GC)
3. How Garbage Collection Works
4. GC Triggers
5. Roots and Reachability
6. Best Practices for Garbage Collection
7. Advanced GC Concepts
8. Tuning and Optimization

### Study Notes (Simplified)
- .NET GC uses generations to collect short-lived objects efficiently.
- Memory pressure comes from allocation rate, object lifetime, and fragmentation.
- Deterministic cleanup is handled with `Dispose`; GC handles memory, not all resources.
