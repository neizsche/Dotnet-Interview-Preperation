...
### Focus
- Core: Core SQL querying and data retrieval syntax.
- Senior Depth: Execution order, null behavior, edge cases, and practical debugging.

### Topic Map
1. SELECT fundamentals
2. WHERE filtering
3. ORDER BY sorting
4. DISTINCT and aliases
5. WHERE vs HAVING
6. NULL semantics
7. Interview corner cases

### Study Notes (Simplified)
- SQL queries are declarative: you state what data you need, not how to retrieve it.
- Accuracy depends on understanding execution order and NULL three-valued logic.
- Most interview mistakes happen in filtering and grouping logic, not syntax.

### Rapid Interview Q&A
- Explain SQL execution order in one example.
- Why does `= NULL` fail and `IS NULL` work?
- Compare `WHERE` and `HAVING` with a grouped query.
