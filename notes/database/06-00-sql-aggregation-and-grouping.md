...
### Focus
- Core: Summarizing data using aggregate functions and grouping logic.
- Senior Depth: HAVING vs WHERE semantics, grouping correctness, and analytical query tuning.
...
7. Advanced grouping concepts

### Study Notes (Simplified)
- Aggregation turns row-level data into business metrics.
- Incorrect grouping can silently produce wrong reports.
- Always validate aggregate logic with small known datasets first.

### Rapid Interview Q&A
- `COUNT(*)` vs `COUNT(column)`: what is the difference?
- `WHERE` vs `HAVING` in grouped queries?
- Why do aggregate queries return unexpected counts?
