...
### Focus
- Core: Using nested queries for filtering, projection, and derived calculations.
- Senior Depth: Correlated subquery behavior, `EXISTS` semantics, and rewrite strategies for performance.

### Topic Map
1. Scalar subqueries
2. Correlated vs non-correlated
3. EXISTS/NOT EXISTS
4. IN/NOT IN with subqueries
5. ANY/ALL patterns
6. Derived tables and CTE alternatives
7. Optimization strategies

### Study Notes (Simplified)
- Subqueries are powerful but must be chosen with execution cost in mind.
- `EXISTS` is often safer for existence checks, especially with NULL edge cases.
- Many subqueries can be rewritten as joins/CTEs for clarity and performance.

### Rapid Interview Q&A
- Correlated vs non-correlated subquery: difference?
- `EXISTS` vs `IN`: when to prefer each?
- Why can `NOT IN` fail with NULLs?
