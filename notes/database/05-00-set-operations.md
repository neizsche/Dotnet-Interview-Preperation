...
### Focus
- Core: Combining result sets using `UNION`, `INTERSECT`, and `EXCEPT`/`MINUS`.
- Senior Depth: Duplicate semantics, column compatibility, ordering rules, and dialect differences.
...
5. NULL behavior
6. ORDER BY restrictions
7. Performance considerations

### Study Notes (Simplified)
- Set operations are declarative tools for merge/common/difference logic.
- `UNION ALL` is usually faster because it skips deduplication.
- Query readability improves when each branch has matching projected schema.

### Rapid Interview Q&A
- `UNION` vs `UNION ALL`: what changes and why?
- Why can set operations fail on data types?
- How does NULL behave in `INTERSECT`/`EXCEPT`?
