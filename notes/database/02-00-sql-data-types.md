...
### Focus
- Core: Choosing correct SQL types for correctness and storage efficiency.
- Senior Depth: Precision/scale trade-offs, cross-database behavior differences, and type-driven performance.

### Topic Map
1. Numeric types
2. Character and Unicode types
3. Date and time types
4. Binary and JSON types
5. Precision and scale rules
6. Conversion pitfalls
7. Dialect differences

### Study Notes (Simplified)
- Data type choices affect accuracy, index size, and query performance.
- `DECIMAL` is safer for financial values; `FLOAT` is approximate by design.
- Explicit conversion is safer than relying on implicit casts.

### Rapid Interview Q&A
- Why avoid `FLOAT` for money?
- Explain `DECIMAL(10,2)` precision and scale.
